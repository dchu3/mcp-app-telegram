...
- `token edit <pair_key> [--min-liquidity ...] [--min-volume ...] [--min-txns ...]`
...
At startup the app syncs `config/scan_set.json` into the SQLite store if it is empty, so every legacy pair becomes editable through the CLI. Any liquidity/volume/transactions thresholds you supply with `token add`, `token set-thresholds`, or the interactive `token edit` workflow are persisted immediately—no restart or code change required.
