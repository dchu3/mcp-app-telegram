...
class EvmMcpClient(McpClient):
    ...
    async def call(self, method: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return await self._stdio_client.call_tool(method, params or {})
    ...
    async def _call_tool_json(self, name: str, arguments: Optional[Mapping[str, Any]] = None) -> Dict[str, Any]:
        ...
        except McpStdioError as exc:  # pragma: no cover - defensive guard
    ...
    async def _fetch_account_mcp(self, address: str) -> AccountSummary:
        ...
        balance_wei_raw = balance.get("wei", 0)
        if isinstance(balance_wei_raw, str):
            balance_wei = int(balance_wei_raw)
        else:
            balance_wei = int(balance_wei_raw or 0)
